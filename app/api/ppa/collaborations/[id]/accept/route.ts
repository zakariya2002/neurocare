...
import { sendCollaborationResponseEmail, sendCollaborationFamilyNotice } from '@/lib/email-collaboration';
...
// POST /api/ppa/collaborations/[id]/accept
  ...
  try {
    ...
    if (child) {
      const { data: family } = await supabase
        ...
        .maybeSingle();
      if (family) {
        const { data: famUser } = await supabase.auth.admin.getUserById(family.user_id);
        if (famUser?.user?.email) {
          await sendCollaborationFamilyNotice({
            to: famUser.user.email,
            familyFirstName: family.first_name || '',
            childFirstName: child.first_name,
            educatorFullName: `${edu.first_name || ''} ${edu.last_name || ''}`.trim(),
            event: 'created',
    ...
    console.error('[ppa-accept] email error', e);
  }
