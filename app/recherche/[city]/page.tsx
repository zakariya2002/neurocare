...
export function generateMetadata({ params }: { params: { city: string } }): Metadata {
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      url: `https://neuro-care.fr/recherche/${params.city}`,
    },
    alternates: {
      canonical: `https://neuro-care.fr/recherche/${params.city}`,
    },
  };
}
      ...
      <div className="min-h-screen bg-[#fdf9f4]">
        ...
        <header className="bg-gradient-to-br from-teal-600 to-teal-700 text-white">
          <div className="max-w-5xl mx-auto px-4 py-12 sm:py-16">
            ...
            <div className="mt-8">
              <Link
                href={`/recherche?location=${encodeURIComponent(city.name)}`}
          ...
          <section className="mb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {SPECIALTIES.map((specialty) => (
                <Link
                  key={specialty}
                  href={`/recherche?location=${encodeURIComponent(city.name)}&q=${encodeURIComponent(specialty)}`}
